...
const precacheComplete = precacheAndRoute(self.__WB_MANIFEST);
precacheComplete.then(() => {
  // After precaching is complete, notify clients
  setTimeout(() => {
    if (!cachingComplete) {
      cachingComplete = true;
      self.clients.matchAll().then((clients) => {
  ...
  }, 3000); // Give a little time for other caching operations
});
